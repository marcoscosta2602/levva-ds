...
import { registryItemFileSchema, registryItemSchema } from "levva/registry"
import { z } from "zod"
...
function BlockViewerToolbar() {
  ...
  return (
    <div className="flex w-full items-center gap-2 pl-2 md:pr-[14px]">
      ...
      <a
        href={`#${item.name}`}
        className="flex-1 text-center text-sm font-sans underline-offset-2 hover:underline md:flex-auto md:text-left"
      >
        {item.description}
      </a>
      <div className="ml-auto hidden items-center gap-2 md:flex">
        ...
        <Button
          ...
          size="sm"
          onClick={() => {
            copyToClipboard(`npx levva@latest add ${item.name}`)
          }}
        >
          {isCopied ? <Check /> : <Terminal />}
          <span>npx levva add {item.name}</span>
        </Button>
        <Separator
          orientation="vertical"
